...
import { getMockEvents, getMockBusinesses, searchEvents, eventCategories } from '../../services/mockData';
...
export default function ExploreScreenEnhanced() {
  ...
  const EventCard = ({ event }: { event: Event }) => (
    <Card onPress={() => console.log('Event pressed:', event.id)} style={styles.itemCard}>
      <View style={styles.cardContent}>
        ...
        <View style={styles.itemInfo}>
          ...
          <Text style={styles.itemLocation} numberOfLines={1}>{event.location}</Text>
  ...
  const BusinessCard = ({ business }: { business: Business }) => (
    <Card onPress={() => console.log('Business pressed:', business.id)} style={styles.itemCard}>
      <View style={styles.cardContent}>
        <View style={styles.businessImagePlaceholder}>
          <Text style={styles.businessImageText}>🏢</Text>
        </View>
        <View style={styles.itemInfo}>
          <Text style={styles.itemTitle} numberOfLines={2}>{business.name}</Text>
          <Text style={styles.businessCategory}>{business.category}</Text>
          <Text style={styles.itemLocation} numberOfLines={1}>{business.location}</Text>
          <View style={styles.itemFooter}>
  ...
  const CategoryChip = ({ category }: { category: typeof eventCategories[0] | { name: string; emoji?: string; color?: string } }) => (
    <TouchableOpacity
      style={[
        styles.categoryChip,
    ...
    <SafeAreaView style={styles.container}>
      ...
      <View style={styles.tabContainer}>
        ...
        <Button
          ...
          size="small"
          style={[styles.tabButton, activeTab === 'businesses' && styles.activeTab]}
        />
      </View>
        ...
        <View style={styles.sectionHeader}>
          <Text style={styles.sectionTitle}>
            {searchText ? 'Search Results' : 
             activeTab === 'events' ? 
               (selectedCategory === 'All' ? 'Popular Events' : `${selectedCategory} Events`) : 
               'Popular Businesses'
            }
          </Text>
          {!loading && (
            <Text style={styles.resultCount}>
              {activeTab === 'events' ? events.length : businesses.length} results
            </Text>
          )}
        </View>
